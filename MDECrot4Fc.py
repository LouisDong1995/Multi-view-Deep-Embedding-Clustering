...
def multiAE(input1_shape=(28, 28, 1), input2_shape=(28, 28, 1), input3_shape=(28, 28, 1), input4_shape=(28, 28, 1), filters=[32, 64, 128, 10]):
    ...
    embedding = Concatenate(axis=1)([embedding1, embedding2, embedding3, embedding4])

    return Model(inputs=[input1, input2, input3, input4], outputs=[deconv1_1, deconv2_1, deconv3_1, deconv4_1], name='AE'), \
           Model(inputs=[input1, input2, input3, input4], outputs=embedding, name='encoder')
...
class MDEC(object):
    def __init__(self,
        ...
        self.multiAE, self.encoder = multiAE(input1_shape, input2_shape, input3_shape, input4_shape, filters)
    ...
    def pretrain(self, x1, x2, x3, x4, batch_size=256, epochs=200, optimizer='adam', save_dir='results/temp', aug_pretrain=False, alpha=0.001):
        print('...Pretraining...')

        loss1 = alpha * (K.sum((K.dot(K.transpose(self.embedding1), self.embedding2))**2) +
                         K.sum((K.dot(K.transpose(self.embedding1), self.embedding3))**2) +
                         K.sum((K.dot(K.transpose(self.embedding1), self.embedding4))**2) +
                         K.sum((K.dot(K.transpose(self.embedding2), self.embedding3))**2) +
                         K.sum((K.dot(K.transpose(self.embedding2), self.embedding4))**2) +
                         K.sum((K.dot(K.transpose(self.embedding3), self.embedding4))**2))

        self.multiAE.compile(optimizer=optimizer, loss=['mse', 'mse', 'mse', 'mse'])
        ...
        t0 = time()
        # if not aug_pretrain:
        #     self.multiAE.fit([x1, x2, x3, x4], [x1, x2, x3, x4], batch_size=batch_size, epochs=epochs, callbacks=[csv_logger])
        # else:
        #     print('-=*' * 20)
        #     print('Using augmentation for ae')
        #     print('-=*' * 20)
        #     def Multi_generator(x1, x2, x3, x4):
        #         gen1 = self.datagen.flow(x1, batch_size=batch_size)
        #         gen2 = self.datagen.flow(x2, batch_size=batch_size)
        #         gen3 = self.datagen.flow(x3, batch_size=batch_size)
        #         gen4 = self.datagen.flow(x4, batch_size=batch_size)
        #         while True:
        #             x1_batch = gen1.next()
        #             x2_batch = gen2.next()
        #             x3_batch = gen3.next()
        #             x4_batch = gen4.next()
        #             yield [x1_batch, x2_batch, x3_batch, x4_batch], [x1_batch, x2_batch, x3_batch, x4_batch]
        #     self.multiAE.fit_generator(Multi_generator(x1, x2, x3, x4), steps_per_epoch=int(x1.shape[0]/batch_size), epochs=epochs, callbacks=[csv_logger])
        print('-=*' * 20)
        print('Using augmentation for ae')
        print('-=*' * 20)
        ...
        self.multiAE.fit_generator(Multi_generator(x1, x2, x3, x4), steps_per_epoch=int(x1.shape[0] / batch_size),
    ...
    def extract_feature(self, x1, x2, x3, x4):  # extract features from before clustering layer
        return self.encoder.predict([x1, x2, x3, x4])

    def predict(self, x1, x2, x3, x4):
        q, d1, d2, d3, d4 = self.model.predict([x1, x2, x3, x4], verbose=0)
        return q.argmax(1)
    ...
    def random_transform(self, x1, x2, x3, x4):
    ...
    def compile(self, loss=['kld', 'mse', 'mse', 'mse', 'mse'], loss_weights=[1, 1, 1, 1, 1], optimizer='adam', alpha=0.001):
        # q, d1, d2, d3, d4 = self.model.predict([x, x_rot90, x_rot180, x_rot270], verbose=0)
        loss1 = alpha * (K.sum((K.dot(K.transpose(self.embedding1), self.embedding2)) ** 2) +
                         K.sum((K.dot(K.transpose(self.embedding1), self.embedding3)) ** 2) +
                         K.sum((K.dot(K.transpose(self.embedding1), self.embedding4)) ** 2) +
                         K.sum((K.dot(K.transpose(self.embedding2), self.embedding3)) ** 2) +
    ...
    def fit(self, x1, x2, x3, x4, y=None, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140, multiAE_weights=None, save_dir='./results/temp', aug_pretrain=False, alpha=0.001):
        ...
        if not self.pretrained and multiAE_weights is None:
            ...
            self.pretrain(x1, x2, x3, x4, batch_size, epochs=args.epochs, save_dir=save_dir, aug_pretrain=aug_pretrain, alpha=alpha)
            self.pretrained = True
        ...
        self.y_pred = kmeans.fit_predict(self.encoder.predict([x1, x2, x3, x4]))
        hidden = self.encoder.predict([x1, x2, x3, x4])
        ...
        print('acc = ', acc, 'x_acc = ', x_acc, 'x_rot90_acc = ', x_rot90_acc, 'x_rot180_acc = ', x_rot180_acc, 'x_rot270_acc = ', x_rot270_acc)
        print('-' * 100)
        ...
        prefile.write('x_rot180_acc = ' + str(x_rot180_acc) + '\n')
        prefile.write('x_rot270_acc = ' + str(x_rot270_acc))
        # return
        ...
        logwriter = csv.DictWriter(logfile, fieldnames=['iter', 'acc', 'nmi', 'ari', 'L', 'Lc', 'L1', 'L2', 'L3', 'L4'])
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                q, _, _, _, _ = self.model.predict([x1, x2, x3, x4], verbose=0)
                ...
                if y is not None:
                    ...
                    logdict = dict(iter=ite, acc=acc, nmi=nmi, ari=ari, L=loss[0], Lc=loss[1], L1=loss[2], L2=loss[3], L3=loss[4], L4=loss[5])
            ...
            if (index + 1) * batch_size > x1.shape[0]:
                ...
                loss = self.model.train_on_batch(x=[x1_batch, x2_batch ,x3_batch,x4_batch],
                                                 y=[p[index * batch_size::], x1_batch, x2_batch, x3_batch, x4_batch])
                index = 0
            else:
                ...
                loss = self.model.train_on_batch(x=[x1_batch, x2_batch, x3_batch,x4_batch],
                                                 y=[p[index * batch_size:(index + 1) * batch_size], x1_batch, x2_batch, x3_batch, x4_batch])
                index += 1
...
if __name__ == "__main__":
    ...
    mdec = MDEC(input1_shape=x.shape[1:], input2_shape=x_rot90.shape[1:], input3_shape=x_rot180.shape[1:], input4_shape=x_rot270.shape[1:], filters=[32, 64, 128, 10], n_clusters=args.n_clusters)
    ...
    mdec.fit(x, x_rot90, x_rot180, x_rot270, y=y, tol=args.tol, maxiter=args.maxiter,
