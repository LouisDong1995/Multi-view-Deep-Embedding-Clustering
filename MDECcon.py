...
def multiAE(input1_shape =(28, 28, 1), input2_shape =(28, 28, 1), filters=[32, 64, 128, 10]):
    ...
    return Model(inputs=[input1, input2], outputs=[deconv1_1, deconv2_1, embedding], name='AE'), \
...
class MDEC(object):
    ...
    def fit(self, x1, x2, y=None, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140, multiAE_weights=None, save_dir='./results/temp'):
        ...
        ori_pred = kmeans.fit_predict(hidden[:10])
        edg_pred = kmeans.fit_predict(hidden[10:])
        ori_acc = metrics.acc(y, ori_pred)
        edg_acc = metrics.acc(y, edg_pred)
        self.model.get_layer(name='clustering').set_weights([kmeans.cluster_centers_])
        print('acc = ', metrics.acc(y, self.y_pred), 'ori_acc = ', ori_acc, 'edg_acc = ', edg_acc)
        print('-' * 100)
        return
