...
def multiAE(input1_shape=(28, 28, 1), input2_shape=(28, 28, 1), filters=[32, 64, 128, 10]):
    ...
    # x = Dense(units=filters[2] * int(input_shape[0] / 8) * int(input_shape[0] / 8), activation='relu')(embedding3)
    ...
    # deconv3_1 = Conv2DTranspose(input_shape[2], 5, strides=2, padding='same', name='deconv3_1')(deconv3_2)
    #
    # # ConvAE4
    ...
    #     deconv4_3)
    # deconv4_1 = Conv2DTranspose(input_shape[2], 5, strides=2, padding='same', name='deconv4_1')(deconv4_2)
...
class MDEC(object):
    ...
    def pretrain(self, x1, x2, batch_size=256, epochs=200, optimizer='adam', save_dir='results/temp', aug_pretrain=False):
        print('...Pretraining...')

        loss1 = K.mean((self.input1 - self.deconv1_1) ** 2) + K.mean((self.input2 - self.deconv2_1) ** 2) + \
               0.001 * K.sum((K.dot(K.transpose(self.embedding1), self.embedding2))**2)
        ...
        if not aug_pretrain:
            ...
            def Multi_generator(x1, x2):
                gen1 = self.datagen.flow(x1, batch_size=batch_size)
                gen2 = self.datagen.flow(x2, batch_size=batch_size)
                while True:
                    x1_batch = gen1.next()
                    x2_batch = gen2.next()
                    yield [x1_batch, x2_batch], [x1_batch, x2_batch]

            self.multiAE.fit_generator(Multi_generator(x1, x2), steps_per_epoch=int(x1.shape[0] / batch_size),
    ...
    def compile(self, optimizer='adam'):
        q, _, _= self.model.predict([x, x_rot90], verbose=0)
    ...
    def fit(self, x1, x2, y=None, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140, multiAE_weights=None, save_dir='./results/temp', aug_pretrain=False):
        ...
        if not self.pretrained and multiAE_weights is None:
            ...
            self.pretrain(x1, x2, batch_size, save_dir=save_dir, aug_pretrain=aug_pretrain)
            self.pretrained = True
        ...
        conv1_acc = metrics.acc(y, kmeans.fit_predict(hidden[:, :10]))
        conv2_acc = metrics.acc(y, kmeans.fit_predict(hidden[:, 10:]))
        print('acc = ', metrics.acc(y, self.y_pred), 'conv1_acc = ', conv1_acc, 'conv2_acc = ', conv2_acc)
        print('-' * 100)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument('--aug_pretrain', action='store_true', help="Whether to use data augmentation during pretraining phase")
    ...
    mdec.fit(x, x_rot90, y=y, tol=args.tol, maxiter=args.maxiter,
             ...
             multiAE_weights=args.multiAE_weights, aug_pretrain=args.aug_pretrain)
    y_pred = mdec.y_pred
    print(
