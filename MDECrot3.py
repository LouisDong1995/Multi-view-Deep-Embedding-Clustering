...
def multiAE(input1_shape=(28, 28, 1), input2_shape=(28, 28, 1), input3_shape=(28, 28, 1), input4_shape=(28, 28, 1), filters=[32, 64, 128, 10]):
    ...
    return Model(inputs=[input1, input2, input3], outputs=[deconv1_1, deconv2_1, deconv3_1, embedding], name='AE'), \
           Model(inputs=[input1, input2, input3], outputs=embedding, name='encoder')
...
class MDEC(object):
    def __init__(self,
        ...
        self.multiAE, self.encoder = multiAE(input1_shape, input2_shape, input3_shape, filters)
    ...
    def pretrain(self, x1, x2, x3, batch_size=256, epochs=200, optimizer='adam', save_dir='results/temp'):
        print('...Pretraining...')

        loss = K.mean((self.input1 - self.deconv1_1) ** 2) + K.mean((self.input2 - self.deconv2_1) ** 2) + K.mean((self.input3 - self.deconv3_1) ** 2) + \
               0.001 * (K.sum((K.dot(K.transpose(self.embedding1), self.embedding2))**2) + K.sum((K.dot(K.transpose(self.embedding1), self.embedding3))**2) +\
        ...
        # begin training
        t0 = time()
        self.multiAE.fit([x1, x2, x3], batch_size=batch_size, epochs=epochs, callbacks=[csv_logger])
    ...
    def extract_feature(self, x1, x2, x3):  # extract features from before clustering layer
        return self.encoder.predict([x1, x2, x3])

    def predict(self, x1, x2, x3):
        q, _, _, _, _= self.model.predict([x1, x2, x3], verbose=0)
        return q.argmax(1)
    ...
    def compile(self, optimizer='adam'):
        q, _, _, _, _ = self.model.predict([x, x_rot90, x_rot180], verbose=0)
        loss = self.target_distribution(q) * K.log(self.target_distribution(q) / q) + \
               K.mean((self.input1 - self.deconv1_1) ** 2) + K.mean((self.input2 - self.deconv2_1) ** 2) + K.mean((self.input3 - self.deconv3_1) ** 2) + \
               0.001 * (K.sum((K.dot(K.transpose(self.embedding1), self.embedding2)) ** 2) + K.sum((K.dot(K.transpose(self.embedding1), self.embedding3)) ** 2) + \
                        K.sum((K.dot(K.transpose(self.embedding2), self.embedding3)) ** 2))
        self.model.add_loss(loss)
        self.model.compile(optimizer=optimizer)

    def fit(self, x1, x2, x3, y=None, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140, multiAE_weights=None, save_dir='./results/temp'):
        ...
        if not self.pretrained and multiAE_weights is None:
            ...
            self.pretrain(x1, x2, x3, batch_size, save_dir=save_dir)
            self.pretrained = True
        ...
        self.y_pred = kmeans.fit_predict(self.encoder.predict([x1, x2, x3]))
        hidden = self.encoder.predict([x1, x2, x3])
        ...
        print('acc = ', metrics.acc(y, self.y_pred), 'x_acc = ', x_acc, 'x_rot90_acc = ', x_rot90_acc, 'x_rot180_acc = ', x_rot180_acc)
        print('-' * 100)
        return
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                q, _, _, _ = self.model.predict([x, x, x], verbose=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('dataset', default='mnist', choices=['mnist', 'usps', 'reutersidf10k', 'mnist_test', 'mnist_rot', 'usps_rot'])
    ...
    mdec = MDEC(input1_shape=x.shape[1:], input2_shape=x_rot90.shape[1:], input3_shape=x_rot180.shape[1:], filters=[32, 64, 128, 10], n_clusters=args.n_clusters)
    ...
    mdec.fit(x, x_rot90, x_rot180, y=y, tol=args.tol, maxiter=args.maxiter,
